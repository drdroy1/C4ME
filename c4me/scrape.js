...
app.get('/college', function (req, res) {
	res.render('admin_college_scrape.ejs');
});
...
app.get('/hs/result', function (req, res) {
	...
	res.render('admin_colleges_scrape_result.ejs', { scrapedCollegeData: req.query.results });
});
...
async function scrape_colleges(res) {
	...
	for (i = 0; i < fsize; ++i) {
		...
		if ($0('li.breadcrumb-item.active').text() == 'College Profile') {
			...
			await page.goto(cd_base_url + '/college/' + cname + '/?tab=profile-academics-tab');
			const cda_content = await page.content();
			const $3 = cheerio.load(cda_content);
			/*
			fetch the academics see more page
			await page.goto(cd_base_url + mpath);
			*/
			$3('h3.h5:contains("Undergraduate Majors") ~ div.row > div.col-sm-6').each(function() {
				let x = $3(this).text();
	...
	console.log('Done');
	res.render('admin_colleges_scrape_result.ejs', { results: added });
}
